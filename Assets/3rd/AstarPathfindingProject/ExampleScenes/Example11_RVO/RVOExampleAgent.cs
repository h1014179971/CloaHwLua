...
namespace Pathfinding.Examples {
	...
	public class RVOExampleAgent : MonoBehaviour {
		...
		public void RecalculatePath () {
			...
			seeker.StartPath(transform.position, target, OnPathComplete, groundMask);
		}

		public void OnPathComplete (Path _p) {
            Debug.Log($"_p===={_p.vectorPath}");
		...
		public void Update () {
            ...
            else if(controller.movementPlane == MovementPlane.XY)
            {
                RaycastHit hit;
                if (Physics.Raycast(pos + Vector3.back, Vector3.forward, out hit, 2, groundMask))
                {
                    pos.y = hit.point.y;
                }
            }
