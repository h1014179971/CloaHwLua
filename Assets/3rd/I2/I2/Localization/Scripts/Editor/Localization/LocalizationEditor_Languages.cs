...
namespace I2.Loc
{
	public partial class LocalizationEditor
	{
		...
		private bool mAppNameTerm_Android;
		private bool mAppNameTerm_Ios;
		private bool mAppATTTerm_Expanded;
        ...
        void OnGUI_StoreIntegration()
        {
			...
			GUILayout.BeginHorizontal();
			mAppNameTerm_Android = GUILayout.Toggle(mAppNameTerm_Android, new GUIContent("App Name Android:", "How should the game be android named in the devices based on their language"), EditorStyles.foldout, GUILayout.Width(160));

			GUILayout.Label("", GUILayout.ExpandWidth(true));
			var rectAndroid = GUILayoutUtility.GetLastRect();
			TermsPopup_Drawer.ShowGUI(rectAndroid, mProp_AppNameAnroid, GUITools.EmptyContent, mLanguageSource);

			if (GUILayout.Button("New Term", EditorStyles.miniButton, GUILayout.ExpandWidth(false)))
			{
				AddLocalTerm("AppName_Android");
				mProp_AppNameAnroid.stringValue = "AppName_Android";
				mAppNameTerm_Android = true;
			}
			GUILayout.EndHorizontal();

			if (mAppNameTerm_Android)
			{
				GUILayout.BeginHorizontal();
				GUILayout.Space(10);

				GUILayout.BeginVertical("Box");
				var termName = mProp_AppNameAnroid.stringValue;
				if (!string.IsNullOrEmpty(termName))
				{
					var termData = LocalizationManager.GetTermData(termName);
					if (termData != null)
						OnGUI_Keys_Languages(mProp_AppNameAnroid.stringValue, ref termData, null, true, mLanguageSource);
				}
				GUILayout.Space(10);
			...
			GUILayout.BeginHorizontal();
			mAppATTTerm_Expanded = GUILayout.Toggle(mAppATTTerm_Expanded, new GUIContent("App ATT translations:", "How should the game be ATT in the devices based on their language"), EditorStyles.foldout, GUILayout.Width(160));

			GUILayout.Label("", GUILayout.ExpandWidth(true));
			var rectATT = GUILayoutUtility.GetLastRect();
			TermsPopup_Drawer.ShowGUI(rectATT, mProp_ATTTerm, GUITools.EmptyContent, mLanguageSource);

			if (GUILayout.Button("New Term", EditorStyles.miniButton, GUILayout.ExpandWidth(false)))
			{
				AddLocalTerm("App_ATT");
				mProp_ATTTerm.stringValue = "App_ATT";
				mAppATTTerm_Expanded = true;
			}
			GUILayout.EndHorizontal();

			if (mAppATTTerm_Expanded)
			{
				GUILayout.BeginHorizontal();
				GUILayout.Space(10);

				GUILayout.BeginVertical("Box");
				var termName = mProp_ATTTerm.stringValue;
				if (!string.IsNullOrEmpty(termName))
				{
					var termData = LocalizationManager.GetTermData(termName);
					if (termData != null)
						OnGUI_Keys_Languages(mProp_ATTTerm.stringValue, ref termData, null, true, mLanguageSource);
				}
				GUILayout.Space(10);

				GUILayout.BeginHorizontal();
				GUILayout.Label("<b>Default App ATT:</b>", lstyle, GUITools.DontExpandWidth);
				GUILayout.Label(Application.productName);
				GUILayout.EndHorizontal();
				GUILayout.EndVertical();
