...
namespace Delivery.Behavior
{
    ...
    public class Stand : Action
    {
        ...
        public override void OnStart()
        {
            ...
            Spine.Animation animation = _skeleton.Data.FindAnimation("stand");
            if (animation == null)
            {
                Debug.LogError($"{Owner.transform.parent.name}没有stand动作");
            }
            else
            {
                _spineAnimationState.SetAnimation(0, "stand", true);
                if (!_flipX)
                    _skeleton.ScaleX = 1;
                else
                    _skeleton.ScaleX = -1;
