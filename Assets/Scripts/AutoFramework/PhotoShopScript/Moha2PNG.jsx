...
function run () {
	...
	var parentName = decodeURI(originalDoc.name);
		parentName = parentName.substring(0, parentName.indexOf("."));
	// create path
	if(!endsWith(saveDir,"/"))
		 saveDir+="/";
	 saveDir = saveDir + parentName + "/";
	 var folder = Folder(saveDir);
	 if(folder.exists)
	 {
...
function showDialog () {
	if (!originalDoc) {
		alert("Please open a document before running the Moha2PNG script.");
		return;
	}
	if (!hasFilePath()) {
		alert("Please save the document before running the Moha2PNG script.");
		return;
	}

	var dialog = new Window("dialog", "Moha2PNG1.0");
	dialog.alignChildren = "fill";
	...
	var checkboxGroup = dialog.add("group");
		var group = checkboxGroup.add("group");
			...
			var writePngsCheckbox = group.add("checkbox", undefined, " 导出图层 PNGs");
			writePngsCheckbox.value = writePngs;
			var writeTemplateCheckbox = group.add("checkbox", undefined, " 写入一张PNG模板");
		...
		group = checkboxGroup.add("group")
			...
			var groupsAsSkinsCheckbox = group.add("checkbox", undefined, " 使用组");
			groupsAsSkinsCheckbox.value = groupsAsSkins;
			 var groupsTrimWhiteSpaceCheckbox = group.add("checkbox", undefined, " 去掉图片空白");
	...
	var slidersGroup = dialog.add("group");
		group = slidersGroup.add("group");
			...
			group.add("statictext", undefined, "PNG 缩放比:");
	...
	var outputGroup = dialog.add("panel", undefined, "导出文件夹");
		outputGroup.alignChildren = "fill";
		outputGroup.margins = [10,15,10,10];
		outputGroup.add("statictext", undefined, "绝对路径 eg:c:/aaa/bb/cc").alignment = "center";
		var textGroup = outputGroup.add("group");
			group = textGroup.add("group");
				group.orientation = "column";
				group.alignChildren = "right";
				group.add("statictext", undefined, "保存路径:");
		...
		var btnBrowse =textGroup.add("button",undefined,"浏览");
	...
	runButton.onClick = function () {
		...
		try {
			...
			alert("An unexpected error has occurred.\n\nTo debug, run the Moha2PNG script using Adobe ExtendScript "
