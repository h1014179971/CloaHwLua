    ...
    public class ResourceManager : MonoBehaviour
    {
        ...
        public static ResourceManager Instance
        {
            get
            {
                if (mInstance == null)
                {
                    ...
                    go.transform.localPosition = new Vector3(10000, 10000, 10000);
        ...
        public void InitPool(string poolName, int size, PoolInflationType type = PoolInflationType.DOUBLE)
        {
            ...
            else
            {
                ...
                AssetRequest request = Assets.LoadAsset(poolName, typeof(GameObject));
                GameObject pb = null;
                if (request.asset != null)
                {
                    pb = request.asset as GameObject;
                }
                //GameObject pb = AssetLoader.Load(poolName, typeof(GameObject)) as GameObject;
                if (pb == null)
                {
                    Debug.LogError("[ResourceManager] Invalide prefab name for pooling :" + poolName);
                    string[] str = poolName.Split('/');
                    string newpoolName = "";
                    string prefabName = "";
                    ...
                    Debug.Log("poolName===" + newpoolName);
                    return;
                }
                else
                {
